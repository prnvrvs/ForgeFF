...
``sqrt``
    Square-root embedding form.

    .. math::

       F(\rho) = \alpha \sqrt{\rho / \beta}

    Parameters: ``alpha``, ``beta``.

How to use a built-in form
--------------------------

Built-in forms have a fixed parameter order. In TOML you only need the form
name and the initial values.
...
.. code-block:: toml
    ...
    backend = "numpy"
    cutoff = 8.0
    initial = [0.03, 5.0, 2.75, 0.01, 2.0, 3.50, 0.0]

If you want the faster JIT pair backend, set:
...
If the analytical form is not in the registry, use ``expression`` and list the
...
- Built-in forms are best when you want a standard parameter order and a fast
  analytical backend.
- Custom expressions are best when you want a one-off equation without editing
  the registry.
- For EAM and ADP, the current TOML support is tabulated rather than analytical.
